...
/**
 * Handles handshakes and messages --- netty示例 websocket处理
 */
public class WebSocketServerHandler extends SimpleChannelInboundHandler<Object> {
    ...
    public void channelRead0(ChannelHandlerContext ctx, Object msg) {
        counter.add(1);
        if (msg instanceof FullHttpRequest) { // websocket握手 协议切换的过程
            // 处理websocket握手 --- 代表新链接
    ...
    private void handleHttpRequest(ChannelHandlerContext ctx, FullHttpRequest req) {
        ...
        if (handshaker == null) {
            ...
            handshaker.handshake(ctx.channel(), req); // 返回告诉客户端，服务器收到你的websocket请求，可用
            ctx.fireChannelRead(req.retain()); // 继续传播 -- 下一个处理器 -- 处理器之间相互触发，前一个处理器
        }
    }
    ...
    private void handleWebSocketFrame(ChannelHandlerContext ctx, WebSocketFrame frame) {
        ...
        if (frame instanceof CloseWebSocketFrame) {
            ...
            UserInfoContext.removeConnection(userId);
